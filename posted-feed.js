...
(function(){
  ...
  function bind(){var vs=[].slice.call(document.querySelectorAll('.kt-public-video'));vs.forEach(function(v){v.onclick=function(){if(v.muted){v.muted=false;v.play().catch(function(){});return}v.paused?v.play().catch(function(){}):v.pause()}});if('IntersectionObserver'in window){var ob=new IntersectionObserver(function(es){es.forEach(function(e){e.isIntersecting&&e.intersectionRatio>.6?e.target.play().catch(function(){}):e.target.pause()})},{threshold:[.6]});vs.forEach(function(v){ob.observe(v)})}}
