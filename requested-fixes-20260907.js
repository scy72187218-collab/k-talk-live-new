...
(function(){
  ...
  document.addEventListener('click',function(e){
    ...
    try{if(window.openEditEffectPanel)window.openEditEffectPanel();}catch(err){}
  },true);
})();
...
(function(){
  ...
  function hasLiveVideo(stream){try{return !!(stream&&stream.getVideoTracks&&stream.getVideoTracks().some(function(t){return t.readyState==='live';}));}catch(e){return false;}}
  function hasLiveAudio(stream){try{return !!(stream&&stream.getAudioTracks&&stream.getAudioTracks().some(function(t){return t.readyState==='live';}));}catch(e){return false;}}
  ...
  async function tryAddMic(stream){
    ...
    try{var a=await navigator.mediaDevices.getUserMedia({video:false,audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});a.getAudioTracks().forEach(function(t){stream.addTrack(t);});}catch(e){}
  }
  window.ensureLiveCamera=async function(facing){
    try{var ok=await oldEnsure.apply(this,arguments);if(window.state&&hasLiveVideo(state.stream))return ok===false?true:ok;}catch(e){}
    ...
    for(var i=0;i<tries.length;i++){
      try{
        ...
        await attachStream(stream);await tryAddMic(stream);return true;
      }catch(e){}
    }
    return false;
  };
...
(function(){
  ...
  function liveStream(){try{var s=window.state&&state.stream;return s&&s.getVideoTracks&&s.getVideoTracks().some(function(t){return t.readyState==='live';})?s:null;}catch(e){return null;}}
  function rememberAllowed(){try{sessionStorage.setItem('kt_camera_allowed_this_session','1');}catch(e){}}
  function creatorVisible(){var c=document.getElementById('creator');return !!(c&&c.classList.contains('show'));}
  async function reviveExisting(){
    var s=liveStream();if(!s)return false;
    try{
      var cam=document.getElementById('camera');var bg=document.getElementById('cameraBg');
      if(cam){if(cam.srcObject!==s)cam.srcObject=s;cam.muted=true;cam.setAttribute('playsinline','');try{await cam.play();}catch(e){}}
      if(bg){if(bg.srcObject!==s)bg.srcObject=s;bg.muted=true;bg.setAttribute('playsinline','');try{await bg.play();}catch(e){}}
      var c=document.getElementById('creator');if(c)c.classList.add('camera-on');rememberAllowed();return true;
    }catch(e){return false;}
  }
  async function permissionState(name){try{if(navigator.permissions&&navigator.permissions.query){var r=await navigator.permissions.query({name:name});return r&&r.state?r.state:'unknown';}}catch(e){}return 'unknown';}
  async function mayReconnectWithoutPrompt(){var cam=await permissionState('camera');if(cam==='granted')return true;if(cam==='prompt'||cam==='denied')return false;try{return sessionStorage.getItem('kt_camera_allowed_this_session')==='1';}catch(e){return false;}}
  async function restoreIfAllowed(){if(await reviveExisting())return true;if(!creatorVisible())return false;if(!(await mayReconnectWithoutPrompt()))return false;if(typeof window.ensureLiveCamera!=='function')return false;try{var ok=await window.ensureLiveCamera((window.state&&state.cameraFacing)||'user');if(await reviveExisting())return true;return !!ok;}catch(e){return false;}}
  var ensureNow=window.ensureLiveCamera;
  if(typeof ensureNow==='function'&&!ensureNow.__ktRememberCameraAllowed){var wrappedEnsure=async function(){var ok=await ensureNow.apply(this,arguments);if(liveStream())rememberAllowed();return ok;};wrappedEnsure.__ktRememberCameraAllowed=true;window.ensureLiveCamera=wrappedEnsure;}
  var openNow=window.openCreator;
  if(typeof openNow==='function'){window.openCreator=async function(){var result=await openNow.apply(this,arguments);setTimeout(function(){restoreIfAllowed();},60);return result;};}
  document.addEventListener('visibilitychange',function(){if(document.visibilityState==='visible'&&creatorVisible())setTimeout(function(){restoreIfAllowed();},120);});
  window.addEventListener('pageshow',function(){if(creatorVisible())setTimeout(function(){restoreIfAllowed();},120);});
})();

/* AI 보정 버튼과 라이브 버튼이 겹쳐 잘못 눌리는 문제만 수정 */
(function(){
  if(window.__ktBeautyLiveTouchSpacingInstalled)return;
  window.__ktBeautyLiveTouchSpacingInstalled=true;
  var s=document.createElement('style');
  s.id='ktBeautyLiveTouchSpacingStyle';
  s.textContent=''
    +'#creator .creator-tools .creator-tool-text[aria-label="AI 보정"]{position:relative!important;z-index:40!important;transform:translateY(-18px)!important;pointer-events:auto!important;touch-action:manipulation!important}'
    +'#creator .creator-tools .creator-tool-text[aria-label="AI 보정"] *{pointer-events:none!important}'
    +'#creator .creator-bottom .modes span:last-child{margin-left:18px!important;position:relative!important;z-index:12!important;touch-action:manipulation!important}'
    +'#creator .creator-bottom .creator-foot span:last-child{margin-left:14px!important;position:relative!important;z-index:12!important;touch-action:manipulation!important}';
  document.head.appendChild(s);
  document.addEventListener('click',function(e){
    var btn=e.target&&e.target.closest?e.target.closest('#creator .creator-tools .creator-tool-text[aria-label="AI 보정"]'):null;
    if(!btn)return;
    e.preventDefault();
    e.stopImmediatePropagation();
