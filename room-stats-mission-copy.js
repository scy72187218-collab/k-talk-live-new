...
(function(){
  ...
  function fillRow(row){
    if(!row)return;
    row.innerHTML='<button type="button" data-kt-room-rank>🔥 일일 랭킹</button><button type="button" data-kt-room-mission>🎯 미션</button><div class="kt-room-viewers-copy">시청자 <b>'+viewerCount()+'</b>명 시청중 🏃</div>';
  ...
  function refreshViewers(){
    var n=viewerCount();
    document.querySelectorAll('.kt-room-viewers-copy').forEach(function(el){el.innerHTML='시청자 <b>'+n+'</b>명 시청중 🏃';});
    document.querySelectorAll('.ktg13-room .ktg13-viewers').forEach(function(el){el.textContent='시청자 '+n+'명이 시청중 🏃';});
  }
