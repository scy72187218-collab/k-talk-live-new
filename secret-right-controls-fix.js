/* 비밀방 오른쪽 버튼만: 뒤집기·좋아요·효과·보물상자·매치를 세로로 유지하고 크기만 조금 키움. */
(function(){
  ...
  function ensureStyle(){
    ...
    s.textContent=''
      +'.ktsecret-right{right:5px!important;bottom:145px!important;gap:6px!important;display:flex!important;flex-direction:column!important;align-items:center!important;z-index:20!important}'
      +'.ktsecret-right>button{width:48px!important;height:48px!important;min-width:48px!important;min-height:48px!important;border-radius:50%!important;font-size:18px!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;padding:0!important}'
      +'.ktsecret-right>.like{height:58px!important;min-height:58px!important;border-radius:18px!important}'
      +'.ktsecret-right>button small,.ktsecret-right>button span{display:block!important;font-size:9px!important;line-height:1!important;margin-top:2px!important;white-space:nowrap!important;font-weight:950!important}'
      +'.ktsecret-right>.like b{font-size:9px!important;line-height:1!important;margin-top:2px!important}'
      +'@media(max-width:390px){.ktsecret-right{right:4px!important;bottom:142px!important;gap:5px!important}.ktsecret-right>button{width:47px!important;height:47px!important;min-width:47px!important;min-height:47px!important}.ktsecret-right>.like{height:56px!important;min-height:56px!important}}';
    document.head.appendChild(s);
  }

  function findButton(side,kind){
    var list=[].slice.call(side.querySelectorAll(':scope > button'));
    return list.find(function(btn){
      var text=String(btn.textContent||'').replace(/\s+/g,'');
      var oc=String(btn.getAttribute('onclick')||'');
      if(kind==='flip')return btn.classList.contains('kt-room-camera-flip')||text.indexOf('뒤집기')>-1;
      if(kind==='like')return btn.classList.contains('like')||text.indexOf('좋아요')>-1;
      ...
      if(kind==='match')return btn.classList.contains('ktsecret-match-restored')||text.indexOf('매치')>-1;
      return false;
    })||null;
  }
