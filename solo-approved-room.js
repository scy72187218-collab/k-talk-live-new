/* K-Talk 1인 방송: 13명방과 같은 출석체크/LED/하단 구성, 게스트칸만 제외. */
(function(){
  ...
  window.ktSoloOpenMessage=function(){
    showSheet('메시지','<div class="rowbox"><b>1인 방송 채팅</b><br>입력한 글이 채팅창에 표시됩니다.</div><input id="ktsoloChatInput" class="form" maxlength="100" placeholder="메시지 입력" onkeydown="if(event.key===\'Enter\')ktSoloSendChat()"><button class="act" onclick="ktSoloSendChat()">보내기</button>');
  ...
  function renderApprovedSolo(){
    ...
    screen.innerHTML='<style id="ktSoloApprovedStyle">'
      ...
      +'.ktsolo-att{justify-self:center;height:43px;min-width:142px;padding:0 9px;border-radius:19px;border:2px solid #ff2bbd;background-color:#130714;background-image:radial-gradient(circle,#ff35ce 1.5px,transparent 2px);background-size:8px 8px;color:#ffd52f;font-size:18px;font-weight:950;box-shadow:0 0 8px #ff2bbd,0 0 18px #ff2bbd66;display:flex;align-items:center;justify-content:center;gap:5px;white-space:nowrap}.ktsolo-att img{width:23px;height:23px;object-fit:contain;filter:drop-shadow(0 0 5px #41b8ff)}'
      +'.ktsolo-air{flex:0 0 36px;display:flex;align-items:center;padding:0 8px;font-size:14px;font-weight:950}.ktsolo-air .on{color:#ff315f;margin-right:7px}.ktsolo-led{flex:0 0 58px;position:relative;border:2px solid #ff28c4;border-radius:22px;background-color:#120712;background-image:radial-gradient(circle,#ff35ce 2px,transparent 2.7px);background-size:13px 13px;overflow:hidden;box-shadow:0 0 9px #ff28c4,0 0 22px #ff28c466}.ktsolo-led-track{position:absolute;left:0;top:0;height:100%;display:flex;align-items:center;white-space:nowrap;will-change:transform;animation:ktsoloMarquee 12s linear infinite;font-size:24px;font-weight:950;color:#ffd62d;text-shadow:0 0 7px #ff8b00}.ktsolo-led-track span{display:inline-block;padding-right:80px}.ktsolo-led-track b{color:#ff59c9}@keyframes ktsoloMarquee{from{transform:translateX(45%)}to{transform:translateX(-100%)}}'
      +'.ktsolo-main{position:relative;flex:1 1 0;min-height:0;overflow:hidden;border-radius:10px;background:#111}.ktsolo-main video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:50% 50%;transform:scaleX(-1);background:#111;filter:brightness(1.08) contrast(.95) saturate(1.02)}.ktsolo-shade{position:absolute;inset:0;background:linear-gradient(180deg,transparent 58%,rgba(0,0,0,.18) 76%,rgba(0,0,0,.64) 100%);pointer-events:none}'
      +'.ktsolo-right{position:absolute;right:8px;bottom:70px;z-index:5;display:grid;gap:7px}.ktsolo-right button{width:48px;height:48px;border-radius:50%;border:1px solid #ffffff38;background:#101014df;color:#fff;font-size:17px;font-weight:950}.ktsolo-right .like{height:56px;border-radius:16px;border-color:#ff65b788;background:#321024dd}.ktsolo-right small{display:block;font-size:8px;margin-top:1px}'
      +'.ktsolo-wave{position:absolute;left:10px;right:62px;bottom:8px;height:48px;z-index:4;display:flex;align-items:end;gap:2px;opacity:.95}.ktsolo-wave i{flex:1;min-width:2px;height:var(--h);border-radius:3px;background:#ff38c6;box-shadow:0 0 5px currentColor;animation:ktsoloWave var(--d) ease-in-out infinite alternate}.ktsolo-wave i:nth-child(6n+2){background:#6f5cff}.ktsolo-wave i:nth-child(6n+3){background:#28d9ff}.ktsolo-wave i:nth-child(6n+4){background:#41e968}.ktsolo-wave i:nth-child(6n+5){background:#ffd43b}.ktsolo-wave i:nth-child(6n){background:#ff774f}@keyframes ktsoloWave{from{transform:scaleY(.45)}to{transform:scaleY(1)}}'
      +'.ktsolo-earn{position:absolute;left:50%;bottom:7px;transform:translateX(-50%);z-index:6;width:44%;max-width:190px}.ktsolo-earn #myEarnHud{position:static!important;transform:none!important;width:100%!important;margin:0!important;padding:4px 7px!important;border:1px solid #d2a936!important;border-radius:14px!important;background:linear-gradient(135deg,#17140bf0,#0d0d12f0)!important;color:#fff!important}'
      +'.ktsolo-chat{flex:0 0 105px;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;padding:4px 8px;background:#000}.ktsolo-chat:empty:before{content:"💬 채팅 메시지가 여기에 표시됩니다";color:#85858b;font-size:11px;font-weight:850;margin:auto 0 4px}.ktsolo-chat-line{display:flex;gap:7px;align-items:baseline;margin-top:4px;font-size:11px;font-weight:850}.ktsolo-chat-line b{color:#65c8ff;font-weight:950}.ktsolo-chat-line span{color:#fff}'
      +'.ktsolo-gifts{flex:0 0 64px;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:3px}.ktsolo-gift{min-width:0;border:1px solid #33343a;border-radius:7px;background:linear-gradient(180deg,#111116,#09090c);color:#fff;padding:2px 1px;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;overflow:hidden}.ktsolo-gift img{width:36px;max-width:84%;height:29px;object-fit:contain;filter:drop-shadow(0 2px 4px #000)}.ktsolo-emoji{height:29px;display:grid;place-items:center;font-size:23px;line-height:1;filter:drop-shadow(0 0 5px #ff5bd4)}.ktsolo-gift b{color:#ffe23e;font-size:8.5px;line-height:1}.ktsolo-gift small{display:block;margin-top:1px;color:#fff;font-size:7px;line-height:1.05;font-weight:900;text-align:center}'
      ...
      +'@media(max-width:390px){.ktsolo-room{padding-left:4px;padding-right:4px;gap:3px}.ktsolo-head{flex-basis:58px;padding:4px 7px}.ktsolo-title{font-size:17px}.ktsolo-brand{font-size:17px}.ktsolo-att{min-width:120px;height:38px;font-size:15px;padding:0 5px}.ktsolo-att img{width:19px;height:19px}.ktsolo-air{flex-basis:31px;font-size:12px}.ktsolo-led{flex-basis:50px}.ktsolo-led-track{font-size:20px}.ktsolo-chat{flex-basis:92px}.ktsolo-gifts{flex-basis:58px}.ktsolo-gift img,.ktsolo-emoji{height:26px}.ktsolo-tools{flex-basis:50px}.ktsolo-tool i{width:33px;height:33px;font-size:16px}}'
      +'</style>'
      +'<section class="ktsolo-room">'
        ...
        +'<div class="ktsolo-main"><video id="ktLiveVideo" autoplay playsinline muted></video><div class="ktsolo-shade"></div>'
        ...
        +'<div class="ktsolo-gifts">'
          +gift('','1개','장미','rose-single.svg')+gift('','50개','장미다발','rose-bouquet-50.svg')+gift('','100개','특대장미','rose-bouquet-100.svg')+gift('💗','10개','하트','')+gift('👑','100개','왕관','')+gift('🏎️','50개','스포츠카','')+gift('','선물상자','큰 선물 보기','gift-box.svg')
        +'</div>'
