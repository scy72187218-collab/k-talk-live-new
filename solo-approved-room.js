...
(function(){
  ...
  function renderApprovedSolo(){
    ...
    screen.innerHTML='<style id="ktSoloApprovedStyle">'
      ...
      +'.ktsolo-room{position:relative;width:100%;height:100dvh;display:flex;flex-direction:column;overflow:hidden;background:#000;color:#fff;font-family:system-ui,-apple-system,"Noto Sans KR",sans-serif;padding:4px 7px calc(5px + env(safe-area-inset-bottom));gap:4px}'
      ...
      +'.ktsolo-main{position:relative;flex:1 1 0;min-height:0;overflow:hidden;border-radius:10px;background:#111}.ktsolo-main #ktLiveVideo{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:50% 50%;transform:scaleX(-1) scale(.90);transform-origin:50% 50%;background:#111;filter:brightness(1.08) contrast(.95) saturate(1.02);z-index:1;border:0;border-radius:0;box-shadow:none}.ktsolo-shade{position:absolute;inset:0;background:linear-gradient(180deg,transparent 50%,rgba(0,0,0,.06) 65%,rgba(0,0,0,.55) 100%);pointer-events:none;z-index:2}'
      ...
      +'.ktsolo-earn{position:absolute;left:auto;right:-8px;top:12px;bottom:auto;transform:scale(.48);transform-origin:top right;z-index:10;width:84px;max-width:18%}.ktsolo-earn #myEarnHud{position:static!important;transform:none!important;width:84px!important;max-width:84px!important;min-width:0!important;margin:0!important;padding:2px!important;border:1px solid #d2a936!important;border-radius:8px!important;background:linear-gradient(135deg,#17140be8,#0d0d12e8)!important;color:#fff!important;overflow:hidden!important}.ktsolo-earn #myEarnHud>div:first-child{gap:1px!important;min-width:0!important}.ktsolo-earn #myEarnHud>div:first-child span{font-size:5px!important;line-height:1.05!important}.ktsolo-earn #hudEarnNet{font-size:7px!important;line-height:1!important}.ktsolo-earn #myEarnDetail{font-size:5px!important;line-height:1.05!important}'
      +'.ktsolo-chat{position:absolute;left:8px;right:120px;bottom:70px;z-index:8;max-height:94px;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;padding:5px 5px 6px;background:transparent;border-radius:9px}.ktsolo-chat:empty:before{content:"💬 채팅 메시지가 여기에 표시됩니다";color:#ddd;font-size:11px;font-weight:850;text-shadow:0 1px 3px #000}.ktsolo-chat-line{display:flex;gap:7px;align-items:baseline;margin-top:4px;font-size:11px;font-weight:850;text-shadow:0 1px 3px #000}.ktsolo-chat-line b{color:#65c8ff;font-weight:950}.ktsolo-chat-line span{color:#fff}'
      +'.ktsolo-gifts{position:absolute;left:3px;right:3px;bottom:60px;z-index:21;height:64px;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:3px}.ktsolo-gift{min-width:0;border:1px solid #ffffff33;border-radius:7px;background:linear-gradient(180deg,rgba(17,17,22,.76),rgba(9,9,12,.84));color:#fff;padding:2px 1px;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;overflow:hidden;backdrop-filter:blur(2px)}.ktsolo-gift img{width:36px;max-width:84%;height:29px;object-fit:contain;filter:drop-shadow(0 2px 4px #000)}.ktsolo-emoji{height:29px;display:grid;place-items:center;font-size:23px;line-height:1;filter:drop-shadow(0 0 5px #ff5bd4)}.ktsolo-gift b{color:#ffe23e;font-size:8.5px;line-height:1}.ktsolo-gift small{display:block;margin-top:1px;color:#fff;font-size:7px;line-height:1.05;font-weight:900;text-align:center}'
      +'.ktsolo-tools{position:absolute;left:7px;right:7px;bottom:calc(5px + env(safe-area-inset-bottom));height:55px;z-index:22;display:grid;grid-template-columns:repeat(8,1fr);gap:2px;align-items:start;background:transparent;pointer-events:auto}.ktsolo-tool{border:0;background:none;color:#fff;min-width:0;font-weight:900;display:grid;justify-items:center;gap:2px}.ktsolo-tool i{width:37px;height:37px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#1b1b20,#0b0b0f);border:1px solid #35363d;font-style:normal;font-size:18px;box-shadow:inset 0 0 13px #ffffff08}.ktsolo-tool:first-child i{box-shadow:0 0 12px #a53fff66,inset 0 0 13px #ffffff08}.ktsolo-tool span{font-size:8px;color:#fff;white-space:nowrap}'
      +'@media(max-width:390px){.ktsolo-room{padding-left:4px;padding-right:4px;gap:3px}.ktsolo-head{flex-basis:58px;padding:4px 7px}.ktsolo-title{font-size:17px}.ktsolo-brand{font-size:17px}.ktsolo-att{min-width:88px;height:28px;font-size:11px;padding:0 3px}.ktsolo-att img{width:14px;height:14px}.ktsolo-air{flex-basis:31px;font-size:12px}.ktsolo-led{flex-basis:50px}.ktsolo-led-track{font-size:20px}.ktsolo-right{bottom:150px}.ktsolo-right button{width:40px;height:40px}.ktsolo-right .like{height:46px}.ktsolo-earn{left:auto;right:-8px;top:10px;bottom:auto;transform:scale(.46);transform-origin:top right;width:82px;max-width:18%}.ktsolo-chat{right:105px;bottom:64px;max-height:88px}.ktsolo-gifts{height:58px}.ktsolo-gift img,.ktsolo-emoji{height:26px}.ktsolo-tools{left:4px;right:4px;height:50px;bottom:calc(4px + env(safe-area-inset-bottom))}.ktsolo-gifts{bottom:54px}.ktsolo-tool i{width:33px;height:33px;font-size:16px}}'
      +'</style>'
  ...
  async function ktSoloKeepCameraWide(){
    try{
      ...
      /* 기존 가로 스트림을 그대로 확대해서 쓰지 않고 1인방만 세로 카메라를 새로 요청 */
      var current=state.stream;
      var needsPortrait=true;
      try{
        var oldTrack=current&&current.getVideoTracks&&current.getVideoTracks()[0];
        var st=oldTrack&&oldTrack.getSettings?oldTrack.getSettings():{};
        var w=Number(st.width||0),h=Number(st.height||0);
        if(w&&h&&h>w)needsPortrait=false;
      }catch(e){}

      if(needsPortrait&&navigator.mediaDevices&&navigator.mediaDevices.getUserMedia){
        try{
          var fresh=await navigator.mediaDevices.getUserMedia({
            video:{
          ...
          if(fresh&&fresh.getVideoTracks&&fresh.getVideoTracks().length){
            try{if(current)current.getTracks().forEach(function(x){x.stop();});}catch(e){}
            state.stream=fresh;
          }else{
            try{fresh&&fresh.getTracks().forEach(function(x){x.stop();});}catch(e){}
          }
        }catch(e){}
      }

      var t=state.stream&&state.stream.getVideoTracks&&state.stream.getVideoTracks()[0];
      if(!t)return;
      try{
        var caps=t.getCapabilities?t.getCapabilities()||{}:{};
        if(caps.zoom&&typeof caps.zoom.min==='number'&&t.applyConstraints){
          var min=Number(caps.zoom.min);
          if(isFinite(min))await t.applyConstraints({advanced:[{zoom:min}]});
        }
      }catch(e){}
    }catch(e){}
  }
