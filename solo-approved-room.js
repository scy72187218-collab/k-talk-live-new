...
(function(){
  ...
  function renderApprovedSolo(){
    ...
    screen.innerHTML='<style id="ktSoloApprovedStyle">'
      ...
      +'.ktsolo-main{position:relative;flex:1 1 0;min-height:0;overflow:hidden;border-radius:10px;background:#111}.ktsolo-main #ktLiveVideo{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:50% 50%;transform:scaleX(-1) scale(.5);transform-origin:50% 50%;background:#111;filter:brightness(1.08) contrast(.95) saturate(1.02);z-index:1}.ktsolo-shade{position:absolute;inset:0;background:linear-gradient(180deg,transparent 50%,rgba(0,0,0,.06) 65%,rgba(0,0,0,.55) 100%);pointer-events:none;z-index:2}'
