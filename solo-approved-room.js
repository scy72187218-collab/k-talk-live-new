...
(function(){
  ...
  function renderApprovedSolo(){
    ...
    screen.innerHTML='<style id="ktSoloApprovedStyle">'
      ...
      +'.ktsolo-main{position:relative;flex:1 1 0;min-height:0;overflow:hidden;border-radius:10px;background:#111}.ktsolo-main .ktsolo-video-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:50% 50%;transform:scaleX(-1);filter:brightness(.92) contrast(.98) saturate(1.01);opacity:1;background:#111}.ktsolo-main #ktLiveVideo{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:50% 50%;transform:scaleX(-1) scale(.91);transform-origin:50% 50%;background:transparent;filter:brightness(1.08) contrast(.95) saturate(1.02);z-index:1;-webkit-mask-image:linear-gradient(to right,transparent 0%,#000 7%,#000 93%,transparent 100%);mask-image:linear-gradient(to right,transparent 0%,#000 7%,#000 93%,transparent 100%)}.ktsolo-shade{position:absolute;inset:0;background:linear-gradient(180deg,transparent 50%,rgba(0,0,0,.06) 65%,rgba(0,0,0,.55) 100%);pointer-events:none;z-index:2}'
      ...
      +'.ktsolo-earn{position:absolute;left:auto;right:135px;bottom:70px;transform:scale(.78);transform-origin:bottom right;z-index:10;width:104px;max-width:28%}.ktsolo-earn #myEarnHud{position:static!important;transform:none!important;width:100%!important;margin:0!important;padding:2px 4px!important;border:1px solid #d2a936!important;border-radius:10px!important;background:linear-gradient(135deg,#17140be8,#0d0d12e8)!important;color:#fff!important}.ktsolo-earn #myEarnHud>div:first-child{gap:2px!important}.ktsolo-earn #myEarnHud>div:first-child span{font-size:6px!important}.ktsolo-earn #hudEarnNet{font-size:9px!important}'
      ...
      +'<section class="ktsolo-room">'
        ...
        +'<div class="ktsolo-main">'
          +'<video id="ktLiveVideoBg" class="ktsolo-video-bg" autoplay playsinline muted></video>'
    ...
    var bg=document.getElementById('ktLiveVideoBg');
    try{
      if(window.state&&state.stream){
        if(bg){
          bg.srcObject=state.stream;
          var bp=bg.play();
