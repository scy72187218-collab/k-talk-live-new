...
/* 설치 안내가 방송 화면을 가리지 않게 하고, 동영상/음악은 다른 화면으로 이동하면 바로 정지 */
(function(){
  if(window.__ktInstallMediaPageFixInstalled)return;
  window.__ktInstallMediaPageFixInstalled=true;

  window.addEventListener('beforeinstallprompt',function(e){
    try{
      e.preventDefault();
      window.__ktDeferredInstallPrompt=e;
    }catch(err){}
  });
  ...
  function hideInstallChip(){
    try{
      ...
      for(var i=0;i<all.length;i++){
        ...
        if(text!=='K-Talk 아이콘 추가'&&text!=='K‑Talk 아이콘 추가')continue;
        var box=el;
        var p=el;
        for(var n=0;n<5&&p&&p!==document.body;n++,p=p.parentElement){
          var cs=getComputedStyle(p);
          var r=p.getBoundingClientRect();
          if((cs.position==='fixed'||cs.position==='sticky')&&r.height>20&&r.height<180){box=p;break;}
        }
  ...
  function stopPageMedia(){
    try{
      document.querySelectorAll('audio,video').forEach(function(m){
        if(m.id==='camera'||m.id==='cameraBg'||m.id==='ktCreatorPreview')return;
        try{m.pause();}catch(e){}
  ...
  function enteringBroadcast(){
    var c=document.getElementById('creator');
    if(c&&c.classList.contains('show'))return true;
    return !!document.querySelector('.ktsolo-room,.ktsubscriber-room,.ktsecret-room,.ktg13-room,.live-prep-open');
  }

  function wrapPageMove(name){
    var fn=window[name];
    if(typeof fn!=='function'||fn.__ktMediaMoveWrapped)return;
    var wrapped=function(){
      stopPageMedia();
      hideInstallChip();
      return fn.apply(this,arguments);
    };
    wrapped.__ktMediaMoveWrapped=true;
    window[name]=wrapped;
  }

  setTimeout(function(){
    ['home','media','friends','openCreator','openRoomPrep','startBroadcast','ktShowDashboard','showDashboard'].forEach(wrapPageMove);
    if(enteringBroadcast())hideInstallChip();
  },0);

  document.addEventListener('click',function(e){
    var t=e.target&&e.target.closest?e.target.closest('.bottom button,.kt-bottom button,.kt-creator-room-shortcuts button,.room-switch,.prep-start'):null;
    if(!t)return;
    stopPageMedia();
    setTimeout(function(){if(enteringBroadcast())hideInstallChip();},0);
  },true);

  var obs=new MutationObserver(function(){
    if(enteringBroadcast())hideInstallChip();
  });
  try{obs.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});}catch(e){}

  document.addEventListener('visibilitychange',function(){
    if(document.visibilityState==='hidden')stopPageMedia();
  });
  window.addEventListener('pagehide',stopPageMedia);
})();
