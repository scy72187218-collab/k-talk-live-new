...
(function(){
  ...
  var hostRoomMissingSince=0,hostLockedVisibleSince=0;
  var hostPeers={};
  var viewerCtx=null;
  ...
  async function startHostPresence(){
    ...
    try{
      ...
      hostActive=true;lastActivityStamp='';hostRoomMissingSince=0;hostLockedVisibleSince=0;
  ...
  /* 방송 화면이 이미 닫혔는데 LIVE 상태만 남는 경우 자동 정리.
     화면 전환 중 잠깐 DOM이 사라지는 것은 방송 종료로 오해하지 않도록 7초 유예한다. */
  ...
  /* 방송 화면은 그대로 열려 있는데 앱 전환/페이지 복귀 때문에 LIVE 등록만 꺼진 경우
     빨간 방송중 표시만 자동 복구한다. 화면/채팅/버튼은 변경하지 않는다. */
  function repairVisibleHostPresence(){
    try{
      if(!hostRoomVisible()||!hasLiveLocalVideo()){hostLockedVisibleSince=0;return;}
      if(hostActive&&hostRoomId){
        /* 복귀 직후 서버 LIVE 상태를 즉시 되살린다. */
      ...
      if(hostEndLock){
        if(!hostLockedVisibleSince)hostLockedVisibleSince=Date.now();
        if(Date.now()-hostLockedVisibleSince<5000)return;
        hostEndLock=false;
