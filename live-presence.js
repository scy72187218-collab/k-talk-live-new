    ...
    try{
      ...
      for(var i=0;i<(rows||[]).length;i++){
        ...
        if(!entry&&x.offer_sdp==='pending'){
          ...
          try{
            var offer=await pc.createOffer({offerToReceiveAudio:false,offerToReceiveVideo:false});await pc.setLocalDescription(offer);await waitIce(pc,320);
  ...
  async function remotePollSignal(){
    ...
    try{
      ...
      if(x.offer_sdp&&x.offer_sdp!=='pending'){
        ...
        var answer=await c.pc.createAnswer();await c.pc.setLocalDescription(answer);await waitIce(c.pc,320);
