(function(){
  function neonCard(edge, glow, iconBg, icon, title, subtitle, onclick){
    return '<button type="button" onclick="'+onclick+'" style="display:flex;min-height:118px;border-radius:22px;padding:16px 14px;align-items:center;gap:12px;text-align:left;color:#fff;background:linear-gradient(145deg,#10111b,#07070d);font-weight:900;border:1.5px solid '+edge+';box-shadow:0 0 18px '+glow+',inset 0 0 24px '+glow+'33;pointer-events:auto;touch-action:manipulation">'
  ...
  window.ktReadCurrentHelp=function(){
    try{
      var title=document.getElementById('sheetTitle');
      var body=document.getElementById('sheetBody');
      var text=((title&&title.textContent)||'')+' '+((body&&body.innerText)||'');
      speakText(text);
    }catch(e){}
  };
  window.ktStopHelpVoice=stopReading;

  function voiceButtons(){
    return '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:0 0 12px">'
      +'<button type="button" onclick="ktReadCurrentHelp()" style="min-height:44px;border:1px solid #78ddff88;border-radius:13px;background:#0a1a27;color:#9feaff;font-weight:950;pointer-events:auto;touch-action:manipulation">🔊 AI 읽어주기</button>'
      +'<button type="button" onclick="ktStopHelpVoice()" style="min-height:44px;border:1px solid #ff9baa88;border-radius:13px;background:#281018;color:#ffc1d0;font-weight:950;pointer-events:auto;touch-action:manipulation">■ 읽기 멈춤</button>'
      +'</div>';
  }
  ...
  window.openMenu=function(){
    var html='<div style="padding:4px 2px 12px">'
      ...
      +'<div style="text-align:center;color:#cfcfe0;font-size:11px;margin-bottom:12px">모르는 기능은 여기서 눌러 바로 확인하세요.</div>'
      +voiceButtons()
      +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
      +neonCard('#ffd46e','#ff9e22','radial-gradient(circle,#ffd85a,#6b4200)','🎁','혜택 · 보상','혜택과 보상 바로 보기','openBenefitHub()')
    ...
    setTimeout(function(){speakText('K-Talk 사용방법과 혜택입니다. 원하는 버튼을 누르세요. AI 읽어주기 버튼을 누르면 현재 화면의 내용을 읽어드립니다.');},60);
  };
  ...
  window.openSubs=function(){
    ...
    setTimeout(function(){speakText('구독과 VIP 혜택 안내입니다. AI 읽어주기 버튼을 누르면 자세한 내용을 읽어드립니다.');},60);
  };

  function installBenefitHubVoice(){
    var old=window.openBenefitHub;
    if(typeof old!=='function'||old.__ktBenefitVoiceWrapped)return false;
    var wrapped=function(){
      var r=old.apply(this,arguments);
      setTimeout(function(){
        try{
          var body=document.getElementById('sheetBody');
          var home=body&&body.querySelector('.kt-benefit-home');
          if(home&&!home.querySelector('[data-kt-benefit-voice]')){
            var bar=document.createElement('div');
            bar.setAttribute('data-kt-benefit-voice','1');
            bar.innerHTML=voiceButtons();
            home.insertBefore(bar,home.firstChild);
          }
          speakText('혜택과 보상 센터입니다. 화면의 버튼을 눌러 원하는 혜택을 확인하세요. AI 읽어주기를 누르면 현재 화면을 읽어드립니다.');
        }catch(e){}
      },30);
      return r;
    };
    wrapped.__ktBenefitVoiceWrapped=true;
  ...
  if(!installBenefitHubVoice()){
    var tries=0;
    var timer=setInterval(function(){
      tries++;
      if(installBenefitHubVoice()||tries>40)clearInterval(timer);
    },100);
  }
})();