...
(function(){
  ...
  function ensureRemoteUi(){
    ...
    if(!document.getElementById('ktRemoteBottom')){var bar=document.createElement('div');bar.id='ktRemoteBottom';bar.className='kt-remote-bottom';bar.innerHTML='<input id="ktRemoteChatInput" maxlength="100" placeholder="입력하세요…" aria-label="라이브 채팅 입력">'+'<button type="button" class="kt-remote-action send" onclick="ktRemoteSendChat()" aria-label="채팅 보내기">➤</button>'+'<button type="button" class="kt-remote-action friend" onclick="if(window.shareApp)shareApp()" aria-label="친구">👥</button>'+'<button type="button" class="kt-remote-action rose" onclick="ktRemoteOpenGifts()" aria-label="장미">🌹</button>'+'<button type="button" class="kt-remote-action gift" onclick="ktRemoteOpenGifts()" aria-label="선물">🎁</button>'+'<button type="button" class="kt-remote-action share" onclick="ktRemoteShare()" aria-label="공유">↗</button>';root.appendChild(bar);var input=bar.querySelector('#ktRemoteChatInput');if(input)input.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();window.ktRemoteSendChat();}});}
    return true;
  }
