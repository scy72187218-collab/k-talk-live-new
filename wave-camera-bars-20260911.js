/* 2026-09-11 요청: 아까 잘 보였던 무지개 막대 파장으로 복구. 1인/13명/구독자/비밀방 파장만 변경. */
(function(){
  if(window.__ktWaveCameraBars20260911Restore)return;
  window.__ktWaveCameraBars20260911Restore=true;

  var st=document.createElement('style');
  st.id='ktWaveCameraBarsStyle20260911Restore';
  st.textContent=''
    ...
    +'/* 1인방: 아래쪽, 카메라 끝에서 끝까지 */'
    +'.ktsolo-main>.kt-open-camera-wave{left:0!important;right:0!important;bottom:66px!important;width:auto!important;height:38px!important;z-index:5!important}'
    +'/* 13명/구독자/비밀방: 열린 카메라 칸 맨 아래 */'
    ...
    +'@media(max-width:390px){.ktsolo-main>.kt-open-camera-wave{bottom:60px!important;height:34px!important}.ktg13-host>.kt-open-camera-wave,.ktg13-guest>.kt-open-camera-wave,.ktsubscriber-host>.kt-open-camera-wave,.ktsubscriber-guest>.kt-open-camera-wave,.ktsecret-slot>.kt-open-camera-wave{height:25px!important}}';
  document.head.appendChild(st);

  function fill(wave){
    if(!wave||wave.dataset.ktBarsRestore==='1')return;
    wave.dataset.ktBarsRestore='1';
    wave.innerHTML='';
    for(var i=0;i<48;i++){
