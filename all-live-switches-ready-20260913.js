...
/* 2026-09-15 라이브 시작 속도만 보강: 5초 카운트는 즉시 5부터 보이고, 카메라는 그 5초 동안 동시에 준비한다. 화면/방/선물/버튼 배치는 변경하지 않음. */
(function(){
  if(window.__ktLiveStartSpeedOnly20260915)return;
  window.__ktLiveStartSpeedOnly20260915=true;

  var originalEnsure=window.ensureLiveCamera;
  ...
  function startWarm(facing){
    ...
    if(typeof originalEnsure!=='function')return Promise.resolve(false);
    try{
      warmPromise=Promise.resolve(originalEnsure.call(window,facing||((window.state&&state.cameraFacing)||'user'))).catch(function(){return false;});
    }catch(e){
  ...
  if(typeof originalEnsure==='function'){
    window.ensureLiveCamera=function(facing){
      if(!startActive)return originalEnsure.apply(this,arguments);
      if(!countdownActive){
        startWarm(facing);
        return Promise.resolve(true);
      }
      return startWarm(facing);
    };
  }
  ...
  if(typeof originalCountdown==='function'){
    window.ktLiveStartCountdown=async function(){
      ...
      var facing=(window.state&&state.cameraFacing)||'user';
      startWarm(facing);
      var result=await originalCountdown.apply(this,arguments);
      if(warmPromise){
        try{await warmPromise;}catch(e){}
      }
      return result;
    };
  }
